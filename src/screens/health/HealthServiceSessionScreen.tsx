...
const VIDEO_STEP_META: Array<{
  ...
  { key: 'confirm_identity', label: 'Confirm Identity', subtitle: 'Verify participant identity.', icon: 'person' },
  { key: 'test_mic_camera', label: 'Test Mic/Camera', subtitle: 'Run pre-session device checks.', icon: 'camera' },
  { key: 'confirm_consent', label: 'Confirm Consent', subtitle: 'Capture telemedicine consent.', icon: 'check' },
  { key: 'join_session', label: 'Join Session', subtitle: 'Join the consultation room.', icon: 'video' },
  { key: 'post_session_summary', label: 'Post Summary', subtitle: 'Submit post-session summary.', icon: 'file' },
];
...
const MESSAGING_STEP_META: Array<{
  ...
  { key: 'open_thread', label: 'Open Thread', subtitle: 'Open secure case thread.', icon: 'chat' },
  { key: 'send_message', label: 'Send Message', subtitle: 'Send first secure message.', icon: 'send' },
  { key: 'attach_files', label: 'Attach Files', subtitle: 'Upload supporting files or voice notes.', icon: 'file' },
  { key: 'close_thread', label: 'Close Thread', subtitle: 'Close conversation after resolution.', icon: 'check' },
];
...
const ADMISSION_STEP_META: Array<{
  ...
  { key: 'admission_reason', label: 'Admission Reason', subtitle: 'Capture reason and initial context.' },
  { key: 'insurance_verification', label: 'Insurance Verification', subtitle: 'Verify insurance and billing path.' },
  { key: 'bed_assignment', label: 'Bed Assignment', subtitle: 'Assign ward and bed details.' },
  { key: 'admission_confirmation', label: 'Admission Confirmation', subtitle: 'Confirm admission and handover.' },
];
...
const EMERGENCY_STEP_META: Array<{
  ...
  { key: 'capture_location', label: 'Capture Location', subtitle: 'Capture incident coordinates.' },
  { key: 'triage_form', label: 'Triage Form', subtitle: 'Assess urgency and patient condition.' },
  { key: 'dispatch_ambulance', label: 'Dispatch Ambulance', subtitle: 'Assign response unit.' },
  { key: 'track_response', label: 'Track Response', subtitle: 'Track movement and arrival.' },
];
...
const PHARMACY_STEP_META: Array<{
  ...
  { key: 'verify_prescription', label: 'Verify Prescription', subtitle: 'Validate prescription and patient profile.' },
  { key: 'validate_inventory', label: 'Validate Inventory', subtitle: 'Confirm stock and substitution policy.' },
  { key: 'confirm_delivery', label: 'Confirm Delivery', subtitle: 'Choose pickup or home delivery.' },
  { key: 'fulfillment_tracking', label: 'Fulfillment Tracking', subtitle: 'Track packing, dispatch, and delivery.' },
];
...
const HOME_LOGISTICS_STEP_META: Array<{
  ...
  { key: 'select_logistics_mode', label: 'Select Mode', subtitle: 'Select nurse visit, pickup, or delivery.' },
  { key: 'schedule_window', label: 'Schedule Window', subtitle: 'Set preferred service window.' },
  { key: 'assign_route', label: 'Assign Route', subtitle: 'Assign rider, route, or nurse.' },
  { key: 'track_eta', label: 'Track ETA', subtitle: 'Track transit and arrival updates.' },
];
...
const WELLNESS_STEP_META: Array<{
  ...
  { key: 'enroll_program', label: 'Enroll Program', subtitle: 'Enroll user into the wellness program.' },
  { key: 'set_goals', label: 'Set Goals', subtitle: 'Set milestones and measurable goals.' },
  { key: 'track_habits', label: 'Track Habits', subtitle: 'Log streaks and habit completion.' },
  { key: 'review_progress', label: 'Review Progress', subtitle: 'Review and confirm progress outcomes.' },
];
...
const REMINDER_STEP_META: Array<{
  ...
  { key: 'select_channels', label: 'Select Channels', subtitle: 'Pick SMS, email, push, or WhatsApp channels.' },
  { key: 'configure_rules', label: 'Configure Rules', subtitle: 'Configure recurrence and reminder rules.' },
  { key: 'schedule_reminders', label: 'Schedule Reminders', subtitle: 'Schedule reminder windows.' },
  { key: 'confirm_delivery', label: 'Confirm Delivery', subtitle: 'Confirm reminder delivery operations.' },
];
...
const toClinicalMap = <T,>(value: T): Record<HealthOpsClinicalEngineCode, T> => ({
  ehr_records: value,
  lab_order: value,
  imaging_order: value,
});
...
export default function HealthServiceSessionScreen({ route, navigation }: Props) {
  ...
  const [messagingEngineMapped, setMessagingEngineMapped] = useState<boolean | null>(null);
  ...
  const [clinicalLoading, setClinicalLoading] = useState<Record<HealthOpsClinicalEngineCode, boolean>>(() =>
    toClinicalMap(false),
  );
  const [clinicalBusy, setClinicalBusy] = useState<Record<HealthOpsClinicalEngineCode, boolean>>(() =>
    toClinicalMap(false),
  );
  const [clinicalErrors, setClinicalErrors] = useState<Record<HealthOpsClinicalEngineCode, string>>(() => toClinicalMap(''));
  const [clinicalSessions, setClinicalSessions] = useState<Record<HealthOpsClinicalEngineCode, any | null>>(() =>
    toClinicalMap(null),
  );
  const [clinicalEngineMapped, setClinicalEngineMapped] = useState<Record<HealthOpsClinicalEngineCode, boolean | null>>(() =>
    toClinicalMap(null),
  );
  const [admissionLoading, setAdmissionLoading] = useState(false);
  const [admissionBusy, setAdmissionBusy] = useState(false);
  const [admissionError, setAdmissionError] = useState('');
  const [admissionEngineMapped, setAdmissionEngineMapped] = useState<boolean | null>(null);
  ...
  const [emergencyEngineMapped, setEmergencyEngineMapped] = useState<boolean | null>(null);
  ...
  const [pharmacyEngineMapped, setPharmacyEngineMapped] = useState<boolean | null>(null);
  ...
  const [billingEngineMapped, setBillingEngineMapped] = useState<boolean | null>(null);
  ...
  const [homeLogisticsEngineMapped, setHomeLogisticsEngineMapped] = useState<boolean | null>(null);
  const [homeLogisticsSession, setHomeLogisticsSession] = useState<any | null>(null);
  const [homeLogisticsTrackingEvents, setHomeLogisticsTrackingEvents] = useState<any[]>([]);
  ...
  const [wellnessEngineMapped, setWellnessEngineMapped] = useState<boolean | null>(null);
  ...
  const [reminderEngineMapped, setReminderEngineMapped] = useState<boolean | null>(null);
  ...
  const videoStepState =
    videoSession?.step_state && typeof videoSession.step_state === 'object' && !Array.isArray(videoSession.step_state)
  ...
  const messagingStatus = String(messagingSession?.status || '').trim().toLowerCase();
  ...
  const messagingIsClosed = messagingStatus === 'completed' || messagingStatus === 'closed' || messagingStatus === 'cancelled';
  const admissionSessionId = String(admissionSession?.id || '').trim();
  const admissionStatus = String(admissionSession?.status || '').trim().toLowerCase();
  const admissionStepState =
    ...
    !Array.isArray(admissionSession.step_state)
      ? admissionSession.step_state
      : {};
  const admissionIsClosed = admissionStatus === 'completed' || admissionStatus === 'cancelled';
  const emergencySessionId = String(emergencySession?.id || '').trim();
  const emergencyStatus = String(emergencySession?.status || '').trim().toLowerCase();
  const emergencyStepState =
    ...
    !Array.isArray(emergencySession.step_state)
      ? emergencySession.step_state
      : {};
  const emergencyIsClosed = emergencyStatus === 'resolved' || emergencyStatus === 'cancelled';
  const recentEmergencyEvents = useMemo(
    () => (Array.isArray(emergencyTrackingEvents) ? emergencyTrackingEvents.slice(-8) : []),
    [emergencyTrackingEvents],
  );
  const pharmacySessionId = String(pharmacySession?.id || '').trim();
  const pharmacyStatus = String(pharmacySession?.status || '').trim().toLowerCase();
  const pharmacyStepState =
    ...
    !Array.isArray(pharmacySession.step_state)
      ? pharmacySession.step_state
      : {};
  const pharmacyIsClosed = pharmacyStatus === 'completed' || pharmacyStatus === 'cancelled';
  const recentPharmacyEvents = useMemo(
    () => (Array.isArray(pharmacyTrackingEvents) ? pharmacyTrackingEvents.slice(-8) : []),
    [pharmacyTrackingEvents],
  );
  const billingSessionId = String(billingSession?.id || '').trim();
  const billingStatus = String(billingSession?.status || '').trim().toLowerCase();
  const billingStepState =
    billingSession?.step_state && typeof billingSession.step_state === 'object' && !Array.isArray(billingSession.step_state)
      ? billingSession.step_state
      : {};
  const billingIsClosed = billingStatus === 'completed' || billingStatus === 'failed' || billingStatus === 'cancelled';
  const homeLogisticsSessionId = String(homeLogisticsSession?.id || '').trim();
  const homeLogisticsStatus = String(homeLogisticsSession?.status || '').trim().toLowerCase();
  const homeLogisticsStepState =
    ...
    !Array.isArray(homeLogisticsSession.step_state)
      ? homeLogisticsSession.step_state
      : {};
  const homeLogisticsIsClosed = homeLogisticsStatus === 'completed' || homeLogisticsStatus === 'cancelled';
  const recentHomeLogisticsEvents = useMemo(
    () => (Array.isArray(homeLogisticsTrackingEvents) ? homeLogisticsTrackingEvents.slice(-8) : []),
    [homeLogisticsTrackingEvents],
  );
  const wellnessSessionId = String(wellnessSession?.id || '').trim();
  const wellnessStatus = String(wellnessSession?.status || '').trim().toLowerCase();
  const wellnessStepState =
    wellnessSession?.step_state && typeof wellnessSession.step_state === 'object' && !Array.isArray(wellnessSession.step_state)
      ? wellnessSession.step_state
      : {};
  const wellnessIsClosed = wellnessStatus === 'completed' || wellnessStatus === 'cancelled';
  const recentWellnessEvents = useMemo(
    () => (Array.isArray(wellnessActivityEvents) ? wellnessActivityEvents.slice(-8) : []),
    [wellnessActivityEvents],
  );
  const reminderSessionId = String(reminderSession?.id || '').trim();
  const reminderStatus = String(reminderSession?.status || '').trim().toLowerCase();
  const reminderStepState =
    reminderSession?.step_state && typeof reminderSession.step_state === 'object' && !Array.isArray(reminderSession.step_state)
      ? reminderSession.step_state
      : {};
  const reminderIsClosed =
    reminderStatus === 'completed' || reminderStatus === 'disabled' || reminderStatus === 'cancelled';
  ...
  const featureRows = useMemo(
    () =>
      [
        ...
        `Scheduled: ${toDateLabel(dateKey)}${timeValue ? ` · ${timeValue}` : ''}`,
        `Standard price: ${toMoney(basePriceCents)}`,
        Number.isFinite(Number(memberPriceCents)) ? `Member price: ${toMoney(memberPriceCents)}` : '',
      ].filter(Boolean),
    ...
    async ({ quiet = false }: { quiet?: boolean } = {}) => {
      ...
      try {
        ...
        if (!response?.success) {
          ...
          throw new Error(response?.message || 'Unable to prepare video consultation.');
        }
        const nextVideoSession = response?.data?.video_session || response?.video_session;
        ...
        const message = error?.message || 'Unable to prepare video consultation.';
  ...
  const refreshVideoSession = useCallback(
    async ({ quiet = false }: { quiet?: boolean } = {}) => {
      ...
      try {
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to refresh video consultation.');
        }
        const nextVideoSession = response?.data?.video_session || response?.video_session;
        if (nextVideoSession && typeof nextVideoSession === 'object') {
          setVideoSession(nextVideoSession);
        ...
        const message = error?.message || 'Unable to refresh video consultation.';
        setVideoError(message);
        if (!quiet) {
          Alert.alert('Video consultation', message);
        }
  ...
  const completeVideoStep = useCallback(
    async (stepKey: HealthOpsVideoStepKey) => {
      ...
      try {
        ...
        Alert.alert('Video consultation', error?.message || 'Unable to update video progress.');
      } finally {
        setVideoBusy(false);
      }
    },
  ...
  const endVideoSession = useCallback(
    async (status: 'completed' | 'cancelled') => {
      ...
      try {
        ...
        Alert.alert('Video consultation', error?.message || 'Unable to end video session.');
      } finally {
        setVideoBusy(false);
      }
    },
  ...
  const openVideoJoinLink = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Video consultation', error?.message || 'Unable to open join link.');
    }
  }, [videoJoinUrl]);

  const setClinicalLoadingFor = useCallback((engineCode: HealthOpsClinicalEngineCode, value: boolean) => {
    setClinicalLoading((prev) => ({ ...prev, [engineCode]: value }));
  }, []);

  const setClinicalBusyFor = useCallback((engineCode: HealthOpsClinicalEngineCode, value: boolean) => {
    setClinicalBusy((prev) => ({ ...prev, [engineCode]: value }));
  }, []);

  const setClinicalErrorFor = useCallback((engineCode: HealthOpsClinicalEngineCode, value: string) => {
    setClinicalErrors((prev) => ({ ...prev, [engineCode]: value }));
  }, []);

  const setClinicalSessionFor = useCallback((engineCode: HealthOpsClinicalEngineCode, session: any | null) => {
    setClinicalSessions((prev) => ({ ...prev, [engineCode]: session }));
  }, []);

  const setClinicalMappedFor = useCallback((engineCode: HealthOpsClinicalEngineCode, value: boolean | null) => {
    setClinicalEngineMapped((prev) => ({ ...prev, [engineCode]: value }));
  }, []);

  const bootstrapMessagingSession = useCallback(
    async ({ quiet = false, markRead = false }: { quiet?: boolean; markRead?: boolean } = {}) => {
      if (!isHealthOpsSession || !cleanWorkflowSessionId) return;
      ...
      try {
        ...
        if (!response?.success) {
          if (Number(response?.status) === 404) {
            ...
            setMessagingError('Secure messaging engine is not mapped to this workflow yet.');
            return;
          }
          throw new Error(response?.message || 'Unable to prepare secure messaging.');
        }
        const nextSession = response?.data?.messaging_session || response?.messaging_session;
        if (nextSession && typeof nextSession === 'object') {
          setMessagingSession(nextSession);
        }
        const fetchedSessionId = String(nextSession?.id || '').trim();
        if (fetchedSessionId) {
          const detail = await fetchHealthOpsMessagingSession(fetchedSessionId, {
            limit: 50,
            markRead,
          });
          if (detail?.success) {
            const detailedSession = detail?.data?.messaging_session || detail?.messaging_session;
            ...
            setMessagingUnreadCount(Number(detail?.data?.unread_count || detail?.unread_count || 0));
          }
        }
  ...
  const refreshMessagingSession = useCallback(
    async ({ quiet = false, markRead = false }: { quiet?: boolean; markRead?: boolean } = {}) => {
      ...
      setMessagingLoading(true);
      try {
        const response = await fetchHealthOpsMessagingSession(messagingSessionId, {
          limit: 50,
          markRead,
        });
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to refresh secure messaging.');
        }
        const nextSession = response?.data?.messaging_session || response?.messaging_session;
        if (nextSession && typeof nextSession === 'object') {
          setMessagingSession(nextSession);
        ...
        const rows = Array.isArray(response?.data?.messages)
          ...
          : Array.isArray(response?.messages)
            ...
            : [];
        setMessagingMessages(rows);
        setMessagingUnreadCount(Number(response?.data?.unread_count || response?.unread_count || 0));
        setMessagingEngineMapped(true);
        setMessagingError('');
      } catch (error: any) {
  ...
  useEffect(() => {
    if (!isHealthOpsSession || !cleanWorkflowSessionId) return;
    bootstrapMessagingSession({ quiet: true, markRead: false }).catch(() => undefined);
  ...
  const completeMessagingStep = useCallback(
    async (stepKey: HealthOpsMessagingStepKey) => {
      ...
      try {
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to update messaging step.');
        }
        const nextSession = response?.data?.messaging_session || response?.messaging_session;
        if (nextSession && typeof nextSession === 'object') {
          setMessagingSession(nextSession);
        ...
        Alert.alert('Secure messaging', error?.message || 'Unable to update messaging step.');
      } finally {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Secure messaging', error?.message || 'Unable to send secure message.');
    } finally {
      setMessagingBusy(false);
    }
  ...
  const endMessagingSession = useCallback(
    async (status: 'completed' | 'closed') => {
      ...
      setMessagingBusy(true);
      try {
        const response = await endHealthOpsMessagingSession(messagingSessionId, {
          status,
          summary:
            status === 'completed'
              ? 'Completed via mobile health service session screen.'
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to end secure messaging session.');
        }
        const nextSession = response?.data?.messaging_session || response?.messaging_session;
        if (nextSession && typeof nextSession === 'object') {
          setMessagingSession(nextSession);
        ...
        Alert.alert('Secure messaging', error?.message || 'Unable to end secure messaging session.');
      } finally {
        setMessagingBusy(false);
      }
    },
  ...
  const bootstrapClinicalSession = useCallback(
      ...
      try {
        ...
        if (!response?.success) {
          if (Number(response?.status) === 404) {
            ...
            setClinicalErrorFor(engineCode, `${CLINICAL_ENGINE_META[engineCode].title} is not mapped to this workflow yet.`);
            return;
          }
          throw new Error(response?.message || `Unable to prepare ${CLINICAL_ENGINE_META[engineCode].title}.`);
        }
        const nextSession = response?.data?.clinical_session || response?.clinical_session;
        if (nextSession && typeof nextSession === 'object') {
        ...
        const message = error?.message || `Unable to prepare ${CLINICAL_ENGINE_META[engineCode].title}.`;
    ...
    [cleanBookingId, cleanWorkflowSessionId, isHealthOpsSession, setClinicalErrorFor, setClinicalLoadingFor, setClinicalMappedFor, setClinicalSessionFor],
  );
  ...
  const refreshClinicalSession = useCallback(
    async (
      ...
      if (!isHealthOpsSession) return;
      const clinicalSessionId = String(clinicalSessions[engineCode]?.id || '').trim();
      ...
      try {
        ...
        if (!response?.success) {
          throw new Error(response?.message || `Unable to refresh ${CLINICAL_ENGINE_META[engineCode].title}.`);
        }
        const nextSession = response?.data?.clinical_session || response?.clinical_session;
        if (nextSession && typeof nextSession === 'object') {
          setClinicalSessionFor(engineCode, nextSession);
        ...
        const message = error?.message || `Unable to refresh ${CLINICAL_ENGINE_META[engineCode].title}.`;
        setClinicalErrorFor(engineCode, message);
        if (!quiet) {
          Alert.alert(CLINICAL_ENGINE_META[engineCode].title, message);
        }
    ...
    [bootstrapClinicalSession, clinicalSessions, isHealthOpsSession, setClinicalErrorFor, setClinicalLoadingFor, setClinicalMappedFor, setClinicalSessionFor],
  );

  useEffect(() => {
    if (!isHealthOpsSession || !cleanWorkflowSessionId) return;
    HEALTH_OPS_CLINICAL_ENGINE_CODES.forEach((engineCode) => {
      bootstrapClinicalSession(engineCode, { quiet: true }).catch(() => undefined);
    });
  ...
  const completeClinicalStep = useCallback(
    async (engineCode: HealthOpsClinicalEngineCode, stepKey: string) => {
      const clinicalSessionId = String(clinicalSessions[engineCode]?.id || '').trim();
      if (!clinicalSessionId) {
        Alert.alert(CLINICAL_ENGINE_META[engineCode].title, 'Clinical engine session is not ready yet.');
        return;
      }
      setClinicalBusyFor(engineCode, true);
      try {
        const response = await updateHealthOpsClinicalStep(clinicalSessionId, stepKey, {
          isCompleted: true,
          payload: {
            source: 'mobile_app',
            completed_at: new Date().toISOString(),
        ...
        if (!response?.success) {
          throw new Error(response?.message || `Unable to update ${CLINICAL_ENGINE_META[engineCode].title} step.`);
        }
        const nextSession = response?.data?.clinical_session || response?.clinical_session;
        if (nextSession && typeof nextSession === 'object') {
          setClinicalSessionFor(engineCode, nextSession);
        ...
        Alert.alert(CLINICAL_ENGINE_META[engineCode].title, error?.message || 'Unable to update clinical step.');
      } finally {
        setClinicalBusyFor(engineCode, false);
      }
    },
    [clinicalSessions, setClinicalBusyFor, setClinicalErrorFor, setClinicalSessionFor],
  );

  const finishClinicalSession = useCallback(
    async (engineCode: HealthOpsClinicalEngineCode, status: 'completed' | 'cancelled') => {
      const clinicalSessionId = String(clinicalSessions[engineCode]?.id || '').trim();
      if (!clinicalSessionId) {
        Alert.alert(CLINICAL_ENGINE_META[engineCode].title, 'Clinical engine session is not ready yet.');
        return;
      ...
      try {
        ...
        if (!response?.success) {
          throw new Error(response?.message || `Unable to finish ${CLINICAL_ENGINE_META[engineCode].title}.`);
        }
        const nextSession = response?.data?.clinical_session || response?.clinical_session;
        if (nextSession && typeof nextSession === 'object') {
          setClinicalSessionFor(engineCode, nextSession);
        ...
        Alert.alert(CLINICAL_ENGINE_META[engineCode].title, error?.message || 'Unable to finish clinical engine session.');
      } finally {
        setClinicalBusyFor(engineCode, false);
      }
    },
      ...
      try {
        ...
        if (!response?.success) {
          if (Number(response?.status) === 404) {
            ...
            setAdmissionError('Admission engine is not mapped to this workflow yet.');
            return;
          }
          throw new Error(response?.message || 'Unable to prepare admission session.');
        }
        const nextSession = response?.data?.admission_session || response?.admission_session;
        if (nextSession && typeof nextSession === 'object') {
        ...
        const message = error?.message || 'Unable to prepare admission session.';
  ...
  const refreshAdmissionSession = useCallback(
    async ({ quiet = false }: { quiet?: boolean } = {}) => {
      ...
      setAdmissionLoading(true);
      try {
        const response = await fetchHealthOpsAdmissionSession(admissionSessionId);
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to refresh admission session.');
        }
        const nextSession = response?.data?.admission_session || response?.admission_session;
        if (nextSession && typeof nextSession === 'object') {
          setAdmissionSession(nextSession);
        ...
        const message = error?.message || 'Unable to refresh admission session.';
        setAdmissionError(message);
        if (!quiet) {
          Alert.alert('Admission & bed', message);
        }
  ...
  const completeAdmissionStep = useCallback(
    async (stepKey: string) => {
      ...
      try {
        const response = await updateHealthOpsAdmissionStep(admissionSessionId, stepKey, {
          isCompleted: true,
          payload: {
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to update admission step.');
        }
        const nextSession = response?.data?.admission_session || response?.admission_session;
        if (nextSession && typeof nextSession === 'object') {
          setAdmissionSession(nextSession);
        ...
        Alert.alert('Admission & bed', error?.message || 'Unable to update admission step.');
      } finally {
  ...
  const finishAdmissionSession = useCallback(
    async (status: 'completed' | 'cancelled') => {
      ...
      setAdmissionBusy(true);
      try {
        const response = await endHealthOpsAdmissionSession(admissionSessionId, {
          status,
          summary:
            status === 'completed'
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to end admission session.');
        }
        const nextSession = response?.data?.admission_session || response?.admission_session;
        if (nextSession && typeof nextSession === 'object') {
          setAdmissionSession(nextSession);
        ...
        Alert.alert('Admission & bed', error?.message || 'Unable to end admission session.');
      } finally {
        setAdmissionBusy(false);
      }
    },
      ...
      try {
        ...
        if (!response?.success) {
          if (Number(response?.status) === 404) {
            ...
            setEmergencyError('Emergency dispatch engine is not mapped to this workflow yet.');
            return;
          }
          throw new Error(response?.message || 'Unable to prepare emergency dispatch session.');
        }
        const nextSession = response?.data?.emergency_session || response?.emergency_session;
        if (nextSession && typeof nextSession === 'object') {
        ...
        const message = error?.message || 'Unable to prepare emergency dispatch session.';
  ...
  const refreshEmergencySession = useCallback(
    async ({ quiet = false }: { quiet?: boolean } = {}) => {
      ...
      setEmergencyLoading(true);
      try {
        const response = await fetchHealthOpsEmergencySession(emergencySessionId, 60);
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to refresh emergency dispatch session.');
        }
        const nextSession = response?.data?.emergency_session || response?.emergency_session;
        if (nextSession && typeof nextSession === 'object') {
          setEmergencySession(nextSession);
        ...
        const message = error?.message || 'Unable to refresh emergency dispatch session.';
        setEmergencyError(message);
        if (!quiet) {
          Alert.alert('Emergency dispatch', message);
        }
  ...
  const completeEmergencyStep = useCallback(
    async (stepKey: string) => {
      ...
      try {
        const response = await updateHealthOpsEmergencyStep(emergencySessionId, stepKey, {
          isCompleted: true,
          payload: {
            source: 'mobile_app',
            completed_at: new Date().toISOString(),
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to update emergency step.');
        }
        const nextSession = response?.data?.emergency_session || response?.emergency_session;
        if (nextSession && typeof nextSession === 'object') {
          setEmergencySession(nextSession);
        ...
        Alert.alert('Emergency dispatch', error?.message || 'Unable to update emergency step.');
      } finally {
  ...
  const sendEmergencyTrackingPing = useCallback(async () => {
    ...
    setEmergencyBusy(true);
    try {
      const response = await updateHealthOpsEmergencyTracking(emergencySessionId, {
        ...(Number.isFinite(etaValue) && etaValue >= 0 ? { etaMinutes: Math.floor(etaValue) } : {}),
        ...(emergencyNoteDraft.trim() ? { note: emergencyNoteDraft.trim() } : {}),
      ...
      if (!response?.success) {
        throw new Error(response?.message || 'Unable to update emergency tracking.');
      }
      const nextSession = response?.data?.emergency_session || response?.emergency_session;
      if (nextSession && typeof nextSession === 'object') {
        setEmergencySession(nextSession);
      ...
      Alert.alert('Emergency dispatch', error?.message || 'Unable to update emergency tracking.');
    } finally {
      setEmergencyBusy(false);
    }
  }, [emergencyEtaDraft, emergencyNoteDraft, emergencySessionId, emergencyStatus]);
  ...
  const finishEmergencySession = useCallback(
    async (status: 'resolved' | 'cancelled') => {
      ...
      setEmergencyBusy(true);
      try {
        const response = await endHealthOpsEmergencySession(emergencySessionId, {
          status,
          summary:
            status === 'resolved'
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to end emergency dispatch session.');
        }
        const nextSession = response?.data?.emergency_session || response?.emergency_session;
        if (nextSession && typeof nextSession === 'object') {
          setEmergencySession(nextSession);
        ...
        Alert.alert('Emergency dispatch', error?.message || 'Unable to end emergency dispatch session.');
      } finally {
        setEmergencyBusy(false);
      }
    },
      ...
      try {
        ...
        if (!response?.success) {
          if (Number(response?.status) === 404) {
            ...
            setPharmacyError('Pharmacy fulfillment engine is not mapped to this workflow yet.');
            return;
          }
          throw new Error(response?.message || 'Unable to prepare pharmacy fulfillment session.');
        }
        const nextSession = response?.data?.pharmacy_session || response?.pharmacy_session;
        if (nextSession && typeof nextSession === 'object') {
        ...
        const message = error?.message || 'Unable to prepare pharmacy fulfillment session.';
  ...
  const refreshPharmacySession = useCallback(
    async ({ quiet = false }: { quiet?: boolean } = {}) => {
      ...
      setPharmacyLoading(true);
      try {
        const response = await fetchHealthOpsPharmacySession(pharmacySessionId, 60);
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to refresh pharmacy fulfillment session.');
        }
        const nextSession = response?.data?.pharmacy_session || response?.pharmacy_session;
        if (nextSession && typeof nextSession === 'object') {
          setPharmacySession(nextSession);
        ...
        const message = error?.message || 'Unable to refresh pharmacy fulfillment session.';
        setPharmacyError(message);
        if (!quiet) {
          Alert.alert('Pharmacy fulfillment', message);
        }
  ...
  const completePharmacyStep = useCallback(
    async (stepKey: string) => {
      ...
      try {
        const response = await updateHealthOpsPharmacyStep(pharmacySessionId, stepKey, {
          isCompleted: true,
          payload: {
            source: 'mobile_app',
            completed_at: new Date().toISOString(),
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to update pharmacy step.');
        }
        const nextSession = response?.data?.pharmacy_session || response?.pharmacy_session;
        if (nextSession && typeof nextSession === 'object') {
          setPharmacySession(nextSession);
        ...
        Alert.alert('Pharmacy fulfillment', error?.message || 'Unable to update pharmacy step.');
      } finally {
  ...
  const sendPharmacyTrackingPing = useCallback(async () => {
    ...
    setPharmacyBusy(true);
    try {
      const response = await updateHealthOpsPharmacyTracking(pharmacySessionId, {
        ...(Number.isFinite(etaValue) && etaValue >= 0 ? { etaMinutes: Math.floor(etaValue) } : {}),
        ...(pharmacyNoteDraft.trim() ? { note: pharmacyNoteDraft.trim() } : {}),
        status: pharmacyStatus === 'inventory_confirmed' ? 'fulfillment_in_progress' : undefined,
        payload: {
          source: 'mobile_app',
          ping_at: new Date().toISOString(),
        },
      ...
      if (!response?.success) {
        throw new Error(response?.message || 'Unable to update pharmacy tracking.');
      }
      const nextSession = response?.data?.pharmacy_session || response?.pharmacy_session;
      if (nextSession && typeof nextSession === 'object') {
        setPharmacySession(nextSession);
      ...
      Alert.alert('Pharmacy fulfillment', error?.message || 'Unable to update pharmacy tracking.');
    } finally {
      setPharmacyBusy(false);
    }
  ...
  const finishPharmacySession = useCallback(
    async (status: 'completed' | 'cancelled') => {
      ...
      try {
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to end pharmacy session.');
        }
        const nextSession = response?.data?.pharmacy_session || response?.pharmacy_session;
        if (nextSession && typeof nextSession === 'object') {
          setPharmacySession(nextSession);
        ...
        Alert.alert('Pharmacy fulfillment', error?.message || 'Unable to end pharmacy session.');
      } finally {
        setPharmacyBusy(false);
      }
    },
      ...
      try {
        ...
        if (!response?.success) {
          if (Number(response?.status) === 404) {
            ...
            setBillingError('Payment & billing engine is not mapped to this workflow yet.');
            return;
          }
          throw new Error(response?.message || 'Unable to prepare billing session.');
        }
        const nextSession = response?.data?.billing_session || response?.billing_session;
        if (nextSession && typeof nextSession === 'object') {
  ...
  const refreshBillingSession = useCallback(
    async ({ quiet = false }: { quiet?: boolean } = {}) => {
      ...
      try {
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to refresh billing session.');
        }
        const nextSession = response?.data?.billing_session || response?.billing_session;
        if (nextSession && typeof nextSession === 'object') {
          setBillingSession(nextSession);
  ...
  const completeBillingStep = useCallback(
    async (stepKey: string) => {
      ...
      try {
        ...
        Alert.alert('Payment & billing', error?.message || 'Unable to update billing step.');
      } finally {
  ...
  const finishBillingSession = useCallback(
    async (status: 'completed' | 'failed' | 'cancelled') => {
      ...
      try {
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to end billing session.');
        }
        const nextSession = response?.data?.billing_session || response?.billing_session;
        if (nextSession && typeof nextSession === 'object') {
          setBillingSession(nextSession);
        ...
        Alert.alert('Payment & billing', error?.message || 'Unable to end billing session.');
      } finally {
        setBillingBusy(false);
      }
    },
      ...
      try {
        ...
        if (!response?.success) {
          if (Number(response?.status) === 404) {
            ...
            setHomeLogisticsError('Home logistics engine is not mapped to this workflow yet.');
            return;
          }
          throw new Error(response?.message || 'Unable to prepare home logistics session.');
        }
        const nextSession = response?.data?.home_logistics_session || response?.home_logistics_session;
        if (nextSession && typeof nextSession === 'object') {
        ...
        const message = error?.message || 'Unable to prepare home logistics session.';
  ...
  const refreshHomeLogisticsSession = useCallback(
    async ({ quiet = false }: { quiet?: boolean } = {}) => {
      ...
      setHomeLogisticsLoading(true);
      try {
        const response = await fetchHealthOpsHomeLogisticsSession(homeLogisticsSessionId, 60);
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to refresh home logistics session.');
        }
        const nextSession = response?.data?.home_logistics_session || response?.home_logistics_session;
        if (nextSession && typeof nextSession === 'object') {
          setHomeLogisticsSession(nextSession);
        ...
        const message = error?.message || 'Unable to refresh home logistics session.';
        setHomeLogisticsError(message);
        if (!quiet) {
          Alert.alert('Home logistics', message);
        }
  ...
  }, [bootstrapHomeLogisticsSession, cleanWorkflowSessionId, isHealthOpsSession]);
  ...
  const completeHomeLogisticsStep = useCallback(
    async (stepKey: string) => {
      ...
      try {
        const response = await updateHealthOpsHomeLogisticsStep(homeLogisticsSessionId, stepKey, {
          isCompleted: true,
          payload: {
            source: 'mobile_app',
            completed_at: new Date().toISOString(),
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to update home logistics step.');
        }
        const nextSession = response?.data?.home_logistics_session || response?.home_logistics_session;
        if (nextSession && typeof nextSession === 'object') {
          setHomeLogisticsSession(nextSession);
        ...
        Alert.alert('Home logistics', error?.message || 'Unable to update home logistics step.');
      } finally {
  ...
  const sendHomeLogisticsTrackingPing = useCallback(async () => {
    ...
    setHomeLogisticsBusy(true);
    try {
      const response = await updateHealthOpsHomeLogisticsTracking(homeLogisticsSessionId, {
        ...(Number.isFinite(etaValue) && etaValue >= 0 ? { etaMinutes: Math.floor(etaValue) } : {}),
        ...(homeLogisticsNoteDraft.trim() ? { note: homeLogisticsNoteDraft.trim() } : {}),
        status: homeLogisticsStatus === 'route_assigned' ? 'in_transit' : undefined,
        payload: {
          source: 'mobile_app',
          ping_at: new Date().toISOString(),
        },
      ...
      if (!response?.success) {
        throw new Error(response?.message || 'Unable to update home logistics tracking.');
      }
      const nextSession = response?.data?.home_logistics_session || response?.home_logistics_session;
      if (nextSession && typeof nextSession === 'object') {
        setHomeLogisticsSession(nextSession);
      ...
      Alert.alert('Home logistics', error?.message || 'Unable to update home logistics tracking.');
    } finally {
      setHomeLogisticsBusy(false);
    }
  }, [homeLogisticsEtaDraft, homeLogisticsNoteDraft, homeLogisticsSessionId, homeLogisticsStatus]);
  ...
  const finishHomeLogisticsSession = useCallback(
    async (status: 'completed' | 'cancelled') => {
      ...
      setHomeLogisticsBusy(true);
      try {
        const response = await endHealthOpsHomeLogisticsSession(homeLogisticsSessionId, {
          status,
          summary:
            status === 'completed'
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to end home logistics session.');
        }
        const nextSession = response?.data?.home_logistics_session || response?.home_logistics_session;
        if (nextSession && typeof nextSession === 'object') {
          setHomeLogisticsSession(nextSession);
        ...
        Alert.alert('Home logistics', error?.message || 'Unable to end home logistics session.');
      } finally {
        setHomeLogisticsBusy(false);
      }
    },
  ...
  const bootstrapWellnessSession = useCallback(
      ...
      try {
        ...
        if (!response?.success) {
          if (Number(response?.status) === 404) {
            ...
            setWellnessError('Wellness program engine is not mapped to this workflow yet.');
            return;
          }
          throw new Error(response?.message || 'Unable to prepare wellness session.');
        }
        const nextSession = response?.data?.wellness_session || response?.wellness_session;
        if (nextSession && typeof nextSession === 'object') {
    ...
    [cleanBookingId, cleanWorkflowSessionId, isHealthOpsSession, wellnessProgramDraft],
  );
  ...
  const refreshWellnessSession = useCallback(
    async ({ quiet = false }: { quiet?: boolean } = {}) => {
      ...
      setWellnessLoading(true);
      try {
        const response = await fetchHealthOpsWellnessSession(wellnessSessionId, 60);
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to refresh wellness session.');
        }
        const nextSession = response?.data?.wellness_session || response?.wellness_session;
        if (nextSession && typeof nextSession === 'object') {
          setWellnessSession(nextSession);
  ...
  const completeWellnessStep = useCallback(
    async (stepKey: string) => {
      ...
      try {
        const completionValue = Number(String(wellnessCompletionDraft || '').trim());
        const response = await updateHealthOpsWellnessStep(wellnessSessionId, stepKey, {
          isCompleted: true,
          payload: {
            source: 'mobile_app',
            completed_at: new Date().toISOString(),
            program_name: wellnessProgramDraft.trim(),
            ...(Number.isFinite(completionValue) ? { completion_percent: Math.max(0, Math.min(100, Math.floor(completionValue))) } : {}),
          },
        });
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to update wellness step.');
        }
        const nextSession = response?.data?.wellness_session || response?.wellness_session;
        if (nextSession && typeof nextSession === 'object') {
          setWellnessSession(nextSession);
        ...
        Alert.alert('Wellness program', error?.message || 'Unable to update wellness step.');
      } finally {
  ...
  const sendWellnessActivityPing = useCallback(async () => {
    ...
    setWellnessBusy(true);
    try {
      const response = await updateHealthOpsWellnessActivity(wellnessSessionId, {
        eventType: 'manual_ping',
        note: wellnessNoteDraft.trim(),
        ...(Number.isFinite(completionValue)
          ? { completionPercent: Math.max(0, Math.min(100, Math.floor(completionValue))) }
          : {}),
        payload: {
          source: 'mobile_app',
          ping_at: new Date().toISOString(),
      ...
      if (!response?.success) {
        throw new Error(response?.message || 'Unable to update wellness activity.');
      }
      const nextSession = response?.data?.wellness_session || response?.wellness_session;
      if (nextSession && typeof nextSession === 'object') {
        setWellnessSession(nextSession);
      ...
      Alert.alert('Wellness program', error?.message || 'Unable to update wellness activity.');
    } finally {
      setWellnessBusy(false);
    }
  }, [wellnessCompletionDraft, wellnessNoteDraft, wellnessProgramDraft, wellnessSessionId]);
  ...
  const finishWellnessSession = useCallback(
    async (status: 'completed' | 'cancelled') => {
      ...
      try {
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to end wellness session.');
        }
        const nextSession = response?.data?.wellness_session || response?.wellness_session;
        if (nextSession && typeof nextSession === 'object') {
          setWellnessSession(nextSession);
        ...
        Alert.alert('Wellness program', error?.message || 'Unable to end wellness session.');
      } finally {
        setWellnessBusy(false);
      }
    },
      ...
      try {
        ...
        if (!response?.success) {
          if (Number(response?.status) === 404) {
            ...
            setReminderError('Notification reminder engine is not mapped to this workflow yet.');
            return;
          }
          throw new Error(response?.message || 'Unable to prepare reminder session.');
        }
        const nextSession = response?.data?.notification_session || response?.notification_session;
        if (nextSession && typeof nextSession === 'object') {
  ...
  const refreshReminderSession = useCallback(
    async ({ quiet = false }: { quiet?: boolean } = {}) => {
      ...
      setReminderLoading(true);
      try {
        const response = await fetchHealthOpsReminderSession(reminderSessionId, 60);
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to refresh reminder session.');
        }
        const nextSession = response?.data?.notification_session || response?.notification_session;
        if (nextSession && typeof nextSession === 'object') {
          setReminderSession(nextSession);
  ...
  const completeReminderStep = useCallback(
    async (stepKey: string) => {
      if (!reminderSessionId) {
        Alert.alert('Notification reminder', 'Reminder session is not ready yet.');
        return;
      }
      setReminderBusy(true);
      try {
        const response = await updateHealthOpsReminderStep(reminderSessionId, stepKey, {
          isCompleted: true,
          payload: {
            source: 'mobile_app',
            completed_at: new Date().toISOString(),
            ...(reminderNextRunDraft.trim() ? { next_run_at: reminderNextRunDraft.trim() } : {}),
          },
        });
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to update reminder step.');
        }
        const nextSession = response?.data?.notification_session || response?.notification_session;
        if (nextSession && typeof nextSession === 'object') {
          setReminderSession(nextSession);
        ...
        Alert.alert('Notification reminder', error?.message || 'Unable to update reminder step.');
      } finally {
  ...
  const sendReminderDeliveryPing = useCallback(async () => {
    ...
    setReminderBusy(true);
    try {
      const response = await updateHealthOpsReminderDelivery(reminderSessionId, {
        ...(reminderNextRunDraft.trim() ? { nextRunAt: reminderNextRunDraft.trim() } : {}),
        sent: true,
      ...
      if (!response?.success) {
        throw new Error(response?.message || 'Unable to update reminder delivery.');
      }
      const nextSession = response?.data?.notification_session || response?.notification_session;
      if (nextSession && typeof nextSession === 'object') {
        setReminderSession(nextSession);
      ...
      Alert.alert('Notification reminder', error?.message || 'Unable to update reminder delivery.');
    } finally {
      setReminderBusy(false);
    }
  }, [reminderNextRunDraft, reminderNoteDraft, reminderSessionId, reminderStatus]);
  ...
  const finishReminderSession = useCallback(
    async (status: 'completed' | 'disabled' | 'cancelled') => {
      ...
      try {
        ...
        if (!response?.success) {
          throw new Error(response?.message || 'Unable to end reminder session.');
        }
        const nextSession = response?.data?.notification_session || response?.notification_session;
        if (nextSession && typeof nextSession === 'object') {
          setReminderSession(nextSession);
        ...
        Alert.alert('Notification reminder', error?.message || 'Unable to end reminder session.');
      } finally {
        setReminderBusy(false);
      }
    },
  ...
  const cancelBooking = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const response = await cancelAppointmentBooking(cleanBookingId, 'Cancelled by user');
      if (!response?.success) {
      ...
      Alert.alert('Appointment', error?.message || 'Unable to cancel appointment.');
    } finally {
      setBusy(false);
    }
  }, [cleanBookingId]);
        ...
        <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xl }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <View style={{ flex: 1, paddingRight: spacing.sm }}>
              <Text style={{ ...typography.h1, color: palette.text }}>Service Session</Text>
          ...
          <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
            <Text style={{ ...typography.h3, color: palette.text }}>{serviceName}</Text>
            <Text style={{ ...typography.body, color: palette.subtext, marginTop: spacing.xs }}>
              {serviceDescription || 'No additional service details.'}
            </Text>
            <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
              {featureRows.map((feature, index) => (
                <Text key={`feature-${index}`} style={{ ...typography.body, color: palette.text }}>
                  • {feature}
                </Text>
              ))}
            </View>
                      ...
                      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <View
                          style={{
                            ...
                            backgroundColor: isActive || isPassed ? `${palette.accentPrimary}33` : `${palette.subtext}22`,
                          }}
                        >
                          <KISIcon
                            name={engine.icon}
                            size={12}
                            color={isActive || isPassed ? palette.accentPrimary : palette.subtext}
                          />
                        </View>
                        <Text style={{ ...typography.label, color: palette.text, marginLeft: spacing.xs }}>
                          {engine.title}
                        </Text>
                      </View>
          ...
          {isHealthOpsSession && activeEngineKey === 'appointment' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Appointment Booking</Text>
                <View
                  style={{
                    borderRadius: 999,
                ...
                >
                  <Text style={{ ...typography.caption, color: palette.accentPrimary }}>Polling</Text>
                </View>
              </View>
              ...
              {appointmentLoading ? (
                <View style={{ marginTop: spacing.sm, flexDirection: 'row', alignItems: 'center' }}>
                  ...
                  <Text style={{ ...typography.body, color: palette.subtext, marginLeft: spacing.xs }}>
              ...
              <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                {bookingRows.map((row, index) => (
                  <Text key={`booking-row-${index}`} style={{ ...typography.body, color: palette.text }}>
                    • {row}
                  </Text>
                ))}
              </View>
              ...
              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={appointmentLoading ? 'Refreshing...' : 'Refresh Booking'}
                  variant="outline"
                  onPress={() => {
                ...
                {appointmentCanMutate ? (
                  <KISButton
                    title={busy ? 'Processing...' : 'Reschedule (Next Available)'}
                    onPress={() => {
          ...
          {isHealthOpsSession && activeEngineKey === 'video' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Video Consultation Engine</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              {videoSession ? (
                <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                  <Text style={{ ...typography.body, color: palette.text }}>
                    Room: {String(videoSession?.room_code || '').trim() || 'Unavailable'}
                  </Text>
                  <Text style={{ ...typography.body, color: palette.text }}>
                    Status: {(videoStatus || 'waiting_room').toUpperCase()}
                  </Text>
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Token expiry: {toDateTimeLabel(videoSession?.token_expires_at)}
                  </Text>
                </View>
              ) : null}
              ...
              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={videoLoading ? 'Preparing...' : videoSessionId ? 'Refresh Video Session' : 'Prepare Video Session'}
                  variant="outline"
                  onPress={() => {
                    refreshVideoSession().catch(() => undefined);
                  }}
                  disabled={videoLoading || videoBusy || !cleanWorkflowSessionId}
                />
                <KISButton
              ...
              {videoEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {VIDEO_STEP_META.map((step) => {
                    const isCompleted = !!videoStepState?.[step.key]?.is_completed;
                    return (
                      <View
                        key={step.key}
                        style={{
                          borderWidth: 1,
                          borderColor: isCompleted ? `${palette.accentPrimary}66` : palette.divider,
                          borderRadius: 12,
                          padding: spacing.sm,
                          backgroundColor: isCompleted ? `${palette.accentPrimary}11` : palette.surface,
                          flexDirection: 'row',
                          alignItems: 'center',
                        }}
                      >
                        <View
                          style={{
                            ...
                            height: 28,
                            borderRadius: 14,
                            backgroundColor: isCompleted ? `${palette.accentPrimary}33` : `${palette.subtext}22`,
                            alignItems: 'center',
                            justifyContent: 'center',
                          }}
                        >
                          <KISIcon name={step.icon} size={14} color={isCompleted ? palette.accentPrimary : palette.subtext} />
                        </View>
                        <View style={{ flex: 1, marginLeft: spacing.sm }}>
                          <Text style={{ ...typography.label, color: palette.text }}>{step.label}</Text>
                          <Text style={{ ...typography.caption, color: palette.subtext }}>{step.subtitle}</Text>
                        </View>
                        <KISButton
                          ...
                          disabled={videoBusy || videoLoading || !videoSessionId || isCompleted || videoStatus === 'cancelled'}
                        />
                      </View>
                    );
                  })}
              ...
              {videoEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <KISButton
                    title={videoBusy ? 'Processing...' : 'Complete Video Session'}
                    onPress={() => {
                      endVideoSession('completed').catch(() => undefined);
                    }}
                    disabled={videoBusy || videoLoading || !videoSessionId || videoStatus === 'completed' || videoStatus === 'cancelled'}
                  />
                  <KISButton
          ...
          {isHealthOpsSession && activeEngineKey === 'messaging' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Secure Messaging Engine</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              {messagingEngineMapped === false ? (
                <Text style={{ ...typography.body, color: palette.subtext, marginTop: spacing.sm }}>
                  Secure messaging engine is not mapped to this service workflow yet.
                </Text>
              ) : null}
              ...
              {messagingSession ? (
                <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                  <Text style={{ ...typography.body, color: palette.text }}>
                    Thread: {String(messagingSession?.thread_code || '').trim() || 'Unavailable'}
                  </Text>
                  <Text style={{ ...typography.body, color: palette.text }}>
              ...
              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={messagingLoading ? 'Preparing...' : messagingSessionId ? 'Refresh Messaging' : 'Prepare Messaging'}
                  variant="outline"
                  onPress={() => {
                    refreshMessagingSession({ markRead: false }).catch(() => undefined);
                  }}
                  disabled={messagingLoading || messagingBusy || !cleanWorkflowSessionId}
                />
                <KISButton
                  title="Mark Messages Read"
                  variant="outline"
                  onPress={() => {
                    refreshMessagingSession({ markRead: true }).catch(() => undefined);
                  }}
                  disabled={messagingLoading || messagingBusy || !messagingSessionId}
                />
              </View>
              ...
              {latestMessages.length ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {latestMessages.map((row: any) => (
                    <View
                      key={String(row?.id || `${row?.created_at || ''}-${row?.body || ''}`)}
                      style={{
                        borderWidth: 1,
                        borderColor: palette.divider,
                        borderRadius: 12,
                    ...
                    >
                      <Text style={{ ...typography.caption, color: palette.subtext }}>
                        {String(row?.message_type || 'text').toUpperCase()} · {toMessageTime(row?.created_at || row?.delivered_at)}
                      </Text>
                      <Text style={{ ...typography.body, color: palette.text, marginTop: 4 }}>
              ...
              {messagingEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md }}>
                  <TextInput
                    ...
                    editable={!messagingBusy && !messagingLoading && !messagingIsClosed}
                  ...
                  <View style={{ marginTop: spacing.xs }}>
                    <KISButton
                      ...
                      disabled={messagingBusy || messagingLoading || !messagingSessionId || messagingIsClosed}
                    />
                  </View>
                </View>
              ) : null}
              ...
              {messagingEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {MESSAGING_STEP_META.map((step) => {
                    const isCompleted = !!messagingStepState?.[step.key]?.is_completed;
                    return (
                      <View
                        key={step.key}
                        style={{
                      ...
                      >
                        ...
                        <KISButton
                          ...
                          variant={isCompleted ? 'outline' : 'primary'}
                          onPress={() => {
                            completeMessagingStep(step.key).catch(() => undefined);
                          }}
                          disabled={messagingBusy || messagingLoading || !messagingSessionId || isCompleted || messagingIsClosed}
                        />
                      </View>
                    );
                  })}
              ...
              {messagingEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <KISButton
                    title={messagingBusy ? 'Processing...' : 'Complete Messaging Session'}
                    onPress={() => {
                  ...
                  />
                  <KISButton
                    title={messagingBusy ? 'Processing...' : 'Close Messaging Session'}
                    variant="outline"
                    onPress={() => {
          ...
          {isHealthOpsSession && activeEngineKey === 'clinical' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Clinical Engines</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              <View style={{ marginTop: spacing.md, gap: spacing.md }}>
                {HEALTH_OPS_CLINICAL_ENGINE_CODES.map((engineCode) => {
                  ...
                  const statusText = String(session?.status || '').trim().toLowerCase();
                  const stepState =
                    session?.step_state && typeof session.step_state === 'object' && !Array.isArray(session.step_state)
                      ? session.step_state
                      : {};
                  const mapped = clinicalEngineMapped[engineCode];
                  const engineTitle = CLINICAL_ENGINE_META[engineCode].title;
                  const engineSubtitle = CLINICAL_ENGINE_META[engineCode].subtitle;
                  const stepKeys = Object.keys(stepState).length
                    ? Object.keys(stepState)
                    : [...HEALTH_OPS_CLINICAL_STEP_META[engineCode]];
                  const isClosed = statusText === 'completed' || statusText === 'cancelled';
                  ...
                  return (
                    ...
                    >
                      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        >
                          <KISIcon name={CLINICAL_ENGINE_META[engineCode].icon} size={16} color={palette.accentPrimary} />
                        </View>
                        <View style={{ flex: 1, marginLeft: spacing.sm }}>
                          <Text style={{ ...typography.label, color: palette.text }}>{engineTitle}</Text>
                          <Text style={{ ...typography.caption, color: palette.subtext }}>{engineSubtitle}</Text>
                        </View>
                      </View>
                      ...
                      <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                        <KISButton
                          ...
                          variant="outline"
                          onPress={() => {
                            refreshClinicalSession(engineCode).catch(() => undefined);
                          }}
                          disabled={clinicalLoading[engineCode] || clinicalBusy[engineCode] || !cleanWorkflowSessionId}
                        />
                      </View>
                      ...
                      {mapped !== false ? (
                        <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                          {stepKeys.map((stepKey) => {
                            const isCompleted = !!stepState?.[stepKey]?.is_completed;
                            return (
                              <View
                              ...
                              >
                                <View style={{ flex: 1 }}>
                                  <Text style={{ ...typography.label, color: palette.text }}>{toStepLabel(stepKey)}</Text>
                                </View>
                                <KISButton
                                  title={isCompleted ? 'Done' : 'Mark Done'}
                                  size="xs"
                                  variant={isCompleted ? 'outline' : 'primary'}
                                  onPress={() => {
                                    completeClinicalStep(engineCode, stepKey).catch(() => undefined);
                                  }}
                                  disabled={
                      ...
                      {mapped !== false ? (
                        <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                          <KISButton
                            title={clinicalBusy[engineCode] ? 'Processing...' : 'Complete Engine'}
                            onPress={() => {
                              finishClinicalSession(engineCode, 'completed').catch(() => undefined);
                            }}
                            disabled={clinicalBusy[engineCode] || clinicalLoading[engineCode] || !clinicalSessionRef || isClosed}
                          />
                          <KISButton
                            title={clinicalBusy[engineCode] ? 'Processing...' : 'Cancel Engine'}
                            variant="outline"
                            onPress={() => {
                              finishClinicalSession(engineCode, 'cancelled').catch(() => undefined);
                            }}
                            disabled={clinicalBusy[engineCode] || clinicalLoading[engineCode] || !clinicalSessionRef || isClosed}
                          />
                        </View>
          ...
          {isHealthOpsSession && activeEngineKey === 'admission' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Admission & Bed Engine</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              {admissionSession ? (
                <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                  ...
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Ward: {String(admissionSession?.ward_name || '').trim() || 'Not assigned'}
                  </Text>
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Bed: {String(admissionSession?.bed_code || '').trim() || 'Not assigned'}
                  </Text>
                </View>
              ) : null}
              ...
              {admissionError ? (
                <Text style={{ ...typography.caption, color: '#EF4444', marginTop: spacing.xs }}>{admissionError}</Text>
              ) : null}

              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={admissionLoading ? 'Preparing...' : admissionSessionId ? 'Refresh Admission' : 'Prepare Admission'}
                  variant="outline"
                  onPress={() => {
                    refreshAdmissionSession().catch(() => undefined);
                  }}
                  disabled={admissionLoading || admissionBusy || !cleanWorkflowSessionId}
                />
              </View>
              ...
              {admissionEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <TextInput
                    ...
                    editable={!admissionBusy && !admissionLoading && !admissionIsClosed}
              ...
              {admissionEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {(Object.keys(admissionStepState).length ? Object.keys(admissionStepState) : [...HEALTH_OPS_ADMISSION_STEP_KEYS]).map(
                    (stepKey) => {
                      const isCompleted = !!admissionStepState?.[stepKey]?.is_completed;
                      const meta = ADMISSION_STEP_META.find((row) => row.key === stepKey);
                      return (
                        <View
                        ...
                        >
                          <View style={{ flex: 1 }}>
                            <Text style={{ ...typography.label, color: palette.text }}>{meta?.label || toStepLabel(stepKey)}</Text>
                            <Text style={{ ...typography.caption, color: palette.subtext }}>
                              {meta?.subtitle || 'Complete this admission workflow step.'}
                            </Text>
                          </View>
                          <KISButton
                            title={isCompleted ? 'Done' : 'Mark Done'}
                            ...
                            variant={isCompleted ? 'outline' : 'primary'}
                            onPress={() => {
                              completeAdmissionStep(stepKey).catch(() => undefined);
                            }}
                            disabled={admissionBusy || admissionLoading || !admissionSessionId || isCompleted || admissionIsClosed}
                          />
                        </View>
                      );
                    },
              ...
              {admissionEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <KISButton
                    title={admissionBusy ? 'Processing...' : 'Complete Admission'}
                    onPress={() => {
                      finishAdmissionSession('completed').catch(() => undefined);
                    }}
                    disabled={admissionBusy || admissionLoading || !admissionSessionId || admissionIsClosed}
                  />
                  <KISButton
                    title={admissionBusy ? 'Processing...' : 'Cancel Admission'}
                    variant="outline"
                    onPress={() => {
                      finishAdmissionSession('cancelled').catch(() => undefined);
                    }}
                    disabled={admissionBusy || admissionLoading || !admissionSessionId || admissionIsClosed}
                  />
                </View>
          ...
          {isHealthOpsSession && activeEngineKey === 'emergency' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Emergency Dispatch Engine</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              {emergencyEngineMapped === false ? (
                <Text style={{ ...typography.body, color: palette.subtext, marginTop: spacing.sm }}>
                  Emergency dispatch engine is not mapped to this service workflow yet.
                </Text>
              ) : null}
              ...
              {emergencySession ? (
                <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                  <Text style={{ ...typography.body, color: palette.text }}>
                    Dispatch: {String(emergencySession?.dispatch_code || '').trim() || 'Unavailable'}
                  </Text>
                  <Text style={{ ...typography.body, color: palette.text }}>
                    Status: {(emergencyStatus || 'waiting').toUpperCase()}
                  </Text>
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    ETA: {Number.isFinite(Number(emergencySession?.current_eta_minutes)) ? `${Number(emergencySession.current_eta_minutes)} min` : 'Not set'}
                  </Text>
                </View>
              ) : null}
              ...
              {emergencyError ? (
                <Text style={{ ...typography.caption, color: '#EF4444', marginTop: spacing.xs }}>{emergencyError}</Text>
              ) : null}

              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={emergencyLoading ? 'Preparing...' : emergencySessionId ? 'Refresh Emergency' : 'Prepare Emergency'}
                  variant="outline"
                  onPress={() => {
                    refreshEmergencySession().catch(() => undefined);
                  }}
                  disabled={emergencyLoading || emergencyBusy || !cleanWorkflowSessionId}
                />
              </View>

              {emergencyEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {(Object.keys(emergencyStepState).length ? Object.keys(emergencyStepState) : [...HEALTH_OPS_EMERGENCY_STEP_KEYS]).map(
                    (stepKey) => {
                      const isCompleted = !!emergencyStepState?.[stepKey]?.is_completed;
                      const meta = EMERGENCY_STEP_META.find((row) => row.key === stepKey);
                      return (
                        <View
                        ...
                        >
                          <View style={{ flex: 1 }}>
                            <Text style={{ ...typography.label, color: palette.text }}>{meta?.label || toStepLabel(stepKey)}</Text>
                            <Text style={{ ...typography.caption, color: palette.subtext }}>
                              {meta?.subtitle || 'Complete this emergency workflow step.'}
                            </Text>
                          </View>
                          <KISButton
                            title={isCompleted ? 'Done' : 'Mark Done'}
                            ...
                            variant={isCompleted ? 'outline' : 'primary'}
                            onPress={() => {
                              completeEmergencyStep(stepKey).catch(() => undefined);
                            }}
                            disabled={emergencyBusy || emergencyLoading || !emergencySessionId || isCompleted || emergencyIsClosed}
                          />
                        </View>
                      );
                    },
              ...
              {emergencyEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <TextInput
                    ...
                    editable={!emergencyBusy && !emergencyLoading && !emergencyIsClosed}
                  ...
                  <KISButton
                    ...
                    disabled={emergencyBusy || emergencyLoading || !emergencySessionId || emergencyIsClosed}
                  />
                </View>
              ) : null}
              ...
              {recentEmergencyEvents.length ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {recentEmergencyEvents.map((event: any, idx: number) => (
                    <View
                      key={`emergency-event-${idx}-${String(event?.timestamp || '')}`}
                      style={{
                        borderWidth: 1,
                        borderColor: palette.divider,
                        borderRadius: 12,
                    ...
                    >
                      <Text style={{ ...typography.caption, color: palette.subtext }}>
                        {String(event?.type || 'update').toUpperCase()} · {toDateTimeLabel(event?.timestamp)}
                      </Text>
                      <Text style={{ ...typography.caption, color: palette.subtext, marginTop: 2 }}>
              ...
              {emergencyEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <KISButton
                    title={emergencyBusy ? 'Processing...' : 'Resolve Emergency'}
                    onPress={() => {
                  ...
                  <KISButton
                    ...
                    variant="outline"
                    onPress={() => {
                      finishEmergencySession('cancelled').catch(() => undefined);
                    }}
                    disabled={emergencyBusy || emergencyLoading || !emergencySessionId || emergencyIsClosed}
                  />
                </View>
          ...
          {isHealthOpsSession && activeEngineKey === 'pharmacy' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Pharmacy & Fulfillment Engine</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              {pharmacyEngineMapped === false ? (
                <Text style={{ ...typography.body, color: palette.subtext, marginTop: spacing.sm }}>
                  Pharmacy fulfillment engine is not mapped to this service workflow yet.
                </Text>
              ) : null}
              ...
              {pharmacySession ? (
                <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                  ...
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Delivery mode: {String(pharmacySession?.delivery_mode || '').trim() || 'Not set'}
                  </Text>
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Payment ref: {String(pharmacySession?.payment_reference || '').trim() || 'Not set'}
                  </Text>
                </View>
              ) : null}
              ...
              {pharmacyError ? (
                <Text style={{ ...typography.caption, color: '#EF4444', marginTop: spacing.xs }}>{pharmacyError}</Text>
              ) : null}

              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={pharmacyLoading ? 'Preparing...' : pharmacySessionId ? 'Refresh Pharmacy' : 'Prepare Pharmacy'}
                  variant="outline"
                  onPress={() => {
                    refreshPharmacySession().catch(() => undefined);
                  }}
                  disabled={pharmacyLoading || pharmacyBusy || !cleanWorkflowSessionId}
                />
              </View>

              {pharmacyEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {(Object.keys(pharmacyStepState).length ? Object.keys(pharmacyStepState) : [...HEALTH_OPS_PHARMACY_STEP_KEYS]).map(
                    (stepKey) => {
                      const isCompleted = !!pharmacyStepState?.[stepKey]?.is_completed;
                      const meta = PHARMACY_STEP_META.find((row) => row.key === stepKey);
                      return (
                        <View
                        ...
                        >
                          <View style={{ flex: 1 }}>
                            <Text style={{ ...typography.label, color: palette.text }}>{meta?.label || toStepLabel(stepKey)}</Text>
                            <Text style={{ ...typography.caption, color: palette.subtext }}>
                              {meta?.subtitle || 'Complete this pharmacy workflow step.'}
                            </Text>
                          </View>
                          <KISButton
                            title={isCompleted ? 'Done' : 'Mark Done'}
                            ...
                            variant={isCompleted ? 'outline' : 'primary'}
                            onPress={() => {
                              completePharmacyStep(stepKey).catch(() => undefined);
                            }}
                            disabled={pharmacyBusy || pharmacyLoading || !pharmacySessionId || isCompleted || pharmacyIsClosed}
                          />
                        </View>
                      );
                    },
              ...
              {pharmacyEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <TextInput
                    ...
                    editable={!pharmacyBusy && !pharmacyLoading && !pharmacyIsClosed}
                    placeholder="ETA minutes (optional)"
                    placeholderTextColor={palette.subtext}
                    keyboardType="numeric"
                    style={{
                  ...
                  />
                  <KISButton
                    title={pharmacyBusy ? 'Sending...' : 'Send Fulfillment Ping'}
                    onPress={() => {
                      sendPharmacyTrackingPing().catch(() => undefined);
                    }}
                    disabled={pharmacyBusy || pharmacyLoading || !pharmacySessionId || pharmacyIsClosed}
                  />
                </View>
              ) : null}
              ...
              {recentPharmacyEvents.length ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {recentPharmacyEvents.map((event: any, idx: number) => (
                    <View
                      key={`pharmacy-event-${idx}-${String(event?.timestamp || '')}`}
                      style={{
                        borderWidth: 1,
                        borderColor: palette.divider,
                        borderRadius: 12,
              ...
              {pharmacyEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <KISButton
                    title={pharmacyBusy ? 'Processing...' : 'Complete Fulfillment'}
                    onPress={() => {
                  ...
                  />
                  <KISButton
                    title={pharmacyBusy ? 'Processing...' : 'Cancel Fulfillment'}
                    variant="outline"
                    onPress={() => {
          ...
          {isHealthOpsSession && activeEngineKey === 'billing' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Payment & Billing Engine</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              {billingEngineMapped === false ? (
                <Text style={{ ...typography.body, color: palette.subtext, marginTop: spacing.sm }}>
                  Payment & billing engine is not mapped to this service workflow yet.
                </Text>
              ) : null}
              ...
              {billingError ? (
                <Text style={{ ...typography.caption, color: '#EF4444', marginTop: spacing.xs }}>{billingError}</Text>
              ) : null}

              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={billingLoading ? 'Preparing...' : billingSessionId ? 'Refresh Billing' : 'Prepare Billing'}
                  variant="outline"
                  onPress={() => {
                    refreshBillingSession().catch(() => undefined);
                  }}
                  disabled={billingLoading || billingBusy || !cleanWorkflowSessionId}
                />
              </View>

              {billingEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {(Object.keys(billingStepState).length ? Object.keys(billingStepState) : [...HEALTH_OPS_BILLING_STEP_KEYS]).map(
                    (stepKey) => {
                      const isCompleted = !!billingStepState?.[stepKey]?.is_completed;
                      const meta = BILLING_STEP_META.find((row) => row.key === stepKey);
                      return (
                        <View
                        ...
                        >
                          <View style={{ flex: 1 }}>
                            <Text style={{ ...typography.label, color: palette.text }}>{meta?.label || toStepLabel(stepKey)}</Text>
                            <Text style={{ ...typography.caption, color: palette.subtext }}>
                              {meta?.subtitle || 'Complete this billing workflow step.'}
                            </Text>
                          </View>
                          <KISButton
                            title={isCompleted ? 'Done' : 'Mark Done'}
                            ...
                            disabled={billingBusy || billingLoading || !billingSessionId || isCompleted || billingIsClosed}
                          />
                        </View>
                      );
                    },
              ...
              {billingEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <KISButton
                    ...
                    disabled={billingBusy || billingLoading || !billingSessionId || billingIsClosed}
                  />
                  <KISButton
                    title={billingBusy ? 'Processing...' : 'Mark Billing Failed'}
                    variant="outline"
                    onPress={() => {
          ...
          {isHealthOpsSession && activeEngineKey === 'home_logistics' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Home Logistics Engine</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              {homeLogisticsEngineMapped === false ? (
                <Text style={{ ...typography.body, color: palette.subtext, marginTop: spacing.sm }}>
                  Home logistics engine is not mapped to this service workflow yet.
                </Text>
              ) : null}
              ...
              {homeLogisticsSession ? (
                <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                  <Text style={{ ...typography.body, color: palette.text }}>
                    Logistics code: {String(homeLogisticsSession?.logistics_code || '').trim() || 'Unavailable'}
                  </Text>
                  <Text style={{ ...typography.body, color: palette.text }}>
                  ...
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    ETA:{' '}
                    {Number.isFinite(Number(homeLogisticsSession?.current_eta_minutes))
                      ? `${Number(homeLogisticsSession.current_eta_minutes)} min`
                      : 'Not set'}
                  </Text>
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Route: {String(homeLogisticsSession?.route_reference || '').trim() || 'Not assigned'}
                  </Text>
                </View>
              ) : null}
              ...
              {homeLogisticsError ? (
                <Text style={{ ...typography.caption, color: '#EF4444', marginTop: spacing.xs }}>{homeLogisticsError}</Text>
              ) : null}

              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={homeLogisticsLoading ? 'Preparing...' : homeLogisticsSessionId ? 'Refresh Logistics' : 'Prepare Logistics'}
                  variant="outline"
                  onPress={() => {
                    refreshHomeLogisticsSession().catch(() => undefined);
                  }}
                  disabled={homeLogisticsLoading || homeLogisticsBusy || !cleanWorkflowSessionId}
                />
              </View>
              ...
              {homeLogisticsEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {(Object.keys(homeLogisticsStepState).length
                    ...
                    const isCompleted = !!homeLogisticsStepState?.[stepKey]?.is_completed;
                    const meta = HOME_LOGISTICS_STEP_META.find((row) => row.key === stepKey);
                    return (
                      <View
                      ...
                      >
                        <View style={{ flex: 1 }}>
                          <Text style={{ ...typography.label, color: palette.text }}>{meta?.label || toStepLabel(stepKey)}</Text>
                          <Text style={{ ...typography.caption, color: palette.subtext }}>
                            {meta?.subtitle || 'Complete this home logistics workflow step.'}
                          </Text>
                        </View>
                        <KISButton
                          title={isCompleted ? 'Done' : 'Mark Done'}
                          ...
                          variant={isCompleted ? 'outline' : 'primary'}
                          onPress={() => {
                            completeHomeLogisticsStep(stepKey).catch(() => undefined);
                          }}
                          disabled={
                            homeLogisticsBusy || homeLogisticsLoading || !homeLogisticsSessionId || isCompleted || homeLogisticsIsClosed
                          }
                        />
                      </View>
                    );
              ...
              {homeLogisticsEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <TextInput
                    ...
                    editable={!homeLogisticsBusy && !homeLogisticsLoading && !homeLogisticsIsClosed}
                    placeholder="ETA minutes (optional)"
                    placeholderTextColor={palette.subtext}
                    keyboardType="numeric"
                    style={{
                  ...
                  />
                  <KISButton
                    title={homeLogisticsBusy ? 'Sending...' : 'Send Logistics Ping'}
                    onPress={() => {
                      sendHomeLogisticsTrackingPing().catch(() => undefined);
                    }}
                    disabled={homeLogisticsBusy || homeLogisticsLoading || !homeLogisticsSessionId || homeLogisticsIsClosed}
                  />
                </View>
              ) : null}
              ...
              {recentHomeLogisticsEvents.length ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {recentHomeLogisticsEvents.map((event: any, idx: number) => (
                    <View
                      key={`home-logistics-event-${idx}-${String(event?.timestamp || '')}`}
                      style={{
                        borderWidth: 1,
                        borderColor: palette.divider,
                        borderRadius: 12,
              ...
              {homeLogisticsEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <KISButton
                    title={homeLogisticsBusy ? 'Processing...' : 'Complete Logistics'}
                    onPress={() => {
                      finishHomeLogisticsSession('completed').catch(() => undefined);
                    }}
                    disabled={homeLogisticsBusy || homeLogisticsLoading || !homeLogisticsSessionId || homeLogisticsIsClosed}
                  />
                  <KISButton
                    title={homeLogisticsBusy ? 'Processing...' : 'Cancel Logistics'}
                    variant="outline"
                    onPress={() => {
                      finishHomeLogisticsSession('cancelled').catch(() => undefined);
                    }}
                    disabled={homeLogisticsBusy || homeLogisticsLoading || !homeLogisticsSessionId || homeLogisticsIsClosed}
                  />
                </View>
          ...
          {isHealthOpsSession && activeEngineKey === 'wellness' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Wellness Program Engine</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              {wellnessEngineMapped === false ? (
                <Text style={{ ...typography.body, color: palette.subtext, marginTop: spacing.sm }}>
                  Wellness program engine is not mapped to this service workflow yet.
                </Text>
              ) : null}
              ...
              {wellnessSession ? (
                <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                  ...
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Program: {String(wellnessSession?.program_name || '').trim() || 'Not set'}
                  </Text>
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Streak: {Number(wellnessSession?.current_streak || 0)}
                  </Text>
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Completion: {Number(wellnessSession?.completion_percent || 0)}%
                  </Text>
                </View>
              ) : null}
              ...
              {wellnessError ? (
                <Text style={{ ...typography.caption, color: '#EF4444', marginTop: spacing.xs }}>{wellnessError}</Text>
              ) : null}

              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={wellnessLoading ? 'Preparing...' : wellnessSessionId ? 'Refresh Wellness' : 'Prepare Wellness'}
                  variant="outline"
                  onPress={() => {
                    refreshWellnessSession().catch(() => undefined);
                  }}
                  disabled={wellnessLoading || wellnessBusy || !cleanWorkflowSessionId}
                />
              </View>
              ...
              {wellnessEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <TextInput
                    ...
                    editable={!wellnessBusy && !wellnessLoading && !wellnessIsClosed}
              ...
              {wellnessEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {(Object.keys(wellnessStepState).length ? Object.keys(wellnessStepState) : [...HEALTH_OPS_WELLNESS_STEP_KEYS]).map(
                    (stepKey) => {
                      const isCompleted = !!wellnessStepState?.[stepKey]?.is_completed;
                      const meta = WELLNESS_STEP_META.find((row) => row.key === stepKey);
                      return (
                        <View
                        ...
                        >
                          <View style={{ flex: 1 }}>
                            <Text style={{ ...typography.label, color: palette.text }}>{meta?.label || toStepLabel(stepKey)}</Text>
                            <Text style={{ ...typography.caption, color: palette.subtext }}>
                              {meta?.subtitle || 'Complete this wellness workflow step.'}
                            </Text>
                          </View>
                          <KISButton
                            title={isCompleted ? 'Done' : 'Mark Done'}
                            ...
                            variant={isCompleted ? 'outline' : 'primary'}
                            onPress={() => {
                              completeWellnessStep(stepKey).catch(() => undefined);
                            }}
                            disabled={wellnessBusy || wellnessLoading || !wellnessSessionId || isCompleted || wellnessIsClosed}
                          />
                        </View>
                      );
                    },
              ...
              {wellnessEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <KISButton
                    ...
                    disabled={wellnessBusy || wellnessLoading || !wellnessSessionId || wellnessIsClosed}
                  />
                </View>
              ) : null}
              ...
              {recentWellnessEvents.length ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {recentWellnessEvents.map((event: any, idx: number) => (
                    <View
                      key={`wellness-event-${idx}-${String(event?.timestamp || '')}`}
                      style={{
                        borderWidth: 1,
                        borderColor: palette.divider,
                        borderRadius: 12,
          ...
          {isHealthOpsSession && activeEngineKey === 'reminder' ? (
            <View style={{ marginTop: spacing.md, borderRadius: spacing.lg, padding: spacing.md, backgroundColor: palette.card, ...borders.card }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ ...typography.h3, color: palette.text }}>Notification & Reminder Engine</Text>
                <View
                  style={{
                    borderRadius: 999,
                    backgroundColor: `${palette.accentPrimary}22`,
              ...
              {reminderSession ? (
                <View style={{ marginTop: spacing.sm, gap: spacing.xs }}>
                  ...
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Next run: {String(reminderSession?.next_run_at || '').trim() || 'Not set'}
                  </Text>
                  <Text style={{ ...typography.caption, color: palette.subtext }}>
                    Sent: {Number(reminderSession?.sent_count || 0)} · Failed: {Number(reminderSession?.failed_count || 0)}
                  </Text>
                </View>
              ) : null}
              ...
              {reminderError ? (
                <Text style={{ ...typography.caption, color: '#EF4444', marginTop: spacing.xs }}>{reminderError}</Text>
              ) : null}

              <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                <KISButton
                  title={reminderLoading ? 'Preparing...' : reminderSessionId ? 'Refresh Reminder' : 'Prepare Reminder'}
                  variant="outline"
                  onPress={() => {
                    refreshReminderSession().catch(() => undefined);
                  }}
                  disabled={reminderLoading || reminderBusy || !cleanWorkflowSessionId}
                />
              </View>

              {reminderEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {(Object.keys(reminderStepState).length ? Object.keys(reminderStepState) : [...HEALTH_OPS_REMINDER_STEP_KEYS]).map(
                    (stepKey) => {
                      const isCompleted = !!reminderStepState?.[stepKey]?.is_completed;
                      const meta = REMINDER_STEP_META.find((row) => row.key === stepKey);
                      return (
                        <View
                        ...
                        >
                          <View style={{ flex: 1 }}>
                            <Text style={{ ...typography.label, color: palette.text }}>{meta?.label || toStepLabel(stepKey)}</Text>
                            <Text style={{ ...typography.caption, color: palette.subtext }}>
                              {meta?.subtitle || 'Complete this reminder workflow step.'}
                            </Text>
                          </View>
                          <KISButton
                            title={isCompleted ? 'Done' : 'Mark Done'}
                            ...
                            variant={isCompleted ? 'outline' : 'primary'}
                            onPress={() => {
                              completeReminderStep(stepKey).catch(() => undefined);
                            }}
                            disabled={reminderBusy || reminderLoading || !reminderSessionId || isCompleted || reminderIsClosed}
                          />
                        </View>
                      );
                    },
              ...
              {reminderEngineMapped !== false ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  <TextInput
                    ...
                    editable={!reminderBusy && !reminderLoading && !reminderIsClosed}
                  ...
                  <KISButton
                    ...
                    disabled={reminderBusy || reminderLoading || !reminderSessionId || reminderIsClosed}
                  />
                </View>
              ) : null}
              ...
              {recentReminderEvents.length ? (
                <View style={{ marginTop: spacing.md, gap: spacing.xs }}>
                  {recentReminderEvents.map((event: any, idx: number) => (
                    <View
                      key={`reminder-event-${idx}-${String(event?.timestamp || '')}`}
                      style={{
                        borderWidth: 1,
                        borderColor: palette.divider,
                        borderRadius: 12,
